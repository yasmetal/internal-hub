...
A single-page internal link hub for **PHATTHA PACKDEE**. It is not an application — it links out to six separately-hosted systems that already exist elsewhere. The entire site is `index.html`: no build step, no package manager, no dependencies, no backend, no login/auth of any kind.
...
**The destination links are hardcoded** inside the card markup in `index.html` (one `<a class="card card-N">` block per system, each with its own gradient color via `--card-N-a`/`--card-N-b` CSS variables). To add/remove/edit a link, edit the card block directly — there's no data-driven list to update elsewhere. The desktop grid breakpoint (`@media (min-width: 900px)`) uses `grid-template-columns: repeat(3, 1fr)`, giving a balanced 3+3 layout for 6 cards; adjust that column count if the number of cards changes.
